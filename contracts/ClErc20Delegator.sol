...
contract ClErc20Delegator is ClTokenInterface, ClErc20Interface, ClDelegatorInterface {
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData)override public {
        require(msg.sender == admin, "ClErc20Delegator::_setImplementation: Caller must be admin");
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, ClTokenInterface clTokenCollateral) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, clTokenCollateral));
        return abi.decode(data, (uint));
    }
    ...
    fallback() external payable {
        require(msg.value == 0,"ClErc20Delegator:fallback: cannot send value to fallback");
